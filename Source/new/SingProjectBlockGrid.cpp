...
ASingProjectBlockGrid::ASingProjectBlockGrid()
{
	...
	struct FConstructorStatics
	{
		...
		FConstructorStatics()
			...
			, BlankMaterial(TEXT("/Game/Puzzle/Meshes/BlackMaterial.BlackMaterial"))
...
void ASingProjectBlockGrid::OnResponseReceived(FHttpRequestPtr RequestP, FHttpResponsePtr Response, bool bWasSuccessful)
{
	...
	if (FJsonSerializer::Deserialize(Reader, JsonObject) &&  JsonObject.IsValid())
	{
			...
			int blur=JsonObject->GetIntegerField("blur");
			...
			else if(intId==4)
			{
				...
				{
					arrLineActorBlanks[idIndex]->right(GreenMaterial, BlueMaterial, BlankMaterial);
				}
				else if(trigger==2)
				{
					arrLineActorBlanks[idIndex]->wrong(RedMaterial, BlueMaterial, BlankMaterial);
				}
