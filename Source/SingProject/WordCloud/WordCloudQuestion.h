...
#include "WordCloudQuestion.generated.h"

UCLASS()
class SINGPROJECT_API AWordCloudQuestion : public AActor
{
	GENERATED_BODY()
...
public:	
	...
	UPROPERTY()
	class AWordCloudMain* OwningGrid;
	...
	UFUNCTION(BlueprintImplementableEvent, Category = Grid)
	void StartAnimateWCPosition();

	UFUNCTION(BlueprintImplementableEvent, Category = Grid)
	void StartAnimateWCPositionOut();

	int intIndex=-1;

	bool boolLineIsMultipleWCPosition;
